...
static const UINT64 MetroF_RoundConstants[24] = 
{
...
void metro1(unsigned char *out, const unsigned char *inraw, unsigned inrawlen)
{
  ...
  for( round_val = 0; round_val < 24; round_val += 2 )
  {
    ...
    BCu = ROL(Asu, 14);
    Eba =   BCa ^((~BCe)&  BCi );
    Eba ^= MetroF_RoundConstants[round_val];
    Ebe =   BCe ^((~BCi)&  BCo );
    Ebi =   BCi ^((~BCo)&  BCu );
    Ebo =   BCo ^((~BCu)&  BCa );
    Ebu =   BCu ^((~BCa)&  BCe );
    ...
    BCu = ROL(Esu, 14);
    Aba =   BCa ^((~BCe)&  BCi );
    Aba ^= MetroF_RoundConstants[round_val+1];
    Abe =   BCe ^((~BCi)&  BCo );
    Abi =   BCi ^((~BCo)&  BCu );
    Abo =   BCo ^((~BCu)&  BCa );
    Abu =   BCu ^((~BCa)&  BCe );
...
void metro_regenhash(struct work *work)
{
  uint256 result;
  unsigned int data[25];
  memcpy(data, work->data, 98);
  metro1((unsigned char*)&result, (unsigned char*)data, 98);
